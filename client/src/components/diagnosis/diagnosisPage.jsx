...
const Diagnosis = observer(() => {
    ...
    const changeInput = (key, value, id) => {
        diagnosis.setDiagnosesInfo(diagnosis.diagnosesInfo.map(item =>
            item.id === id ?
                {...item, [key]: value}
                : item))
        console.log(diagnosis.diagnosesInfo)
    }
    ...
    const addDiagnosis = () => {
        diagnosis.setDiagnosesInfo([...diagnosis.diagnosesInfo, {
            complaints: '',
            objective: '',
            probing: '',
            treatment: '',
    ...
    return (
        ...
        <>
            <button className={style.btn_add} onClick={addDiagnosis}>Добавить диагноз</button>
            <div className={style.main_list}>
                {diagnosis.diagnosesInfo.map(blockItem =>
